...
bool Maze::solve(int row, int col)
{
    if (maze[row][col] == FINISH)
        ...
        return false; 
    else if (row < 0 or col < 0 or row >= MAX_MAZE_SIZE or col >= MAX_MAZE_SIZE)
        return false; 
    ...
    else if (maze[row][col] == REJECT)
        return false; 

    maze[row][col] = PATH; 

    return (solve(row - 1, col) or solve(row, col + 1) or solve(row + 1, col) or solve(row, col - 1));
